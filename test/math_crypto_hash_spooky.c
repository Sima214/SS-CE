...
#define SPOOKY_HASH_RESULTS_BUFSIZE 512
#define SPOOKY_HASH_ALIGNMENT_BUFSIZE 1024
int spooky_hash_test_results() {
  printf("Testing results...");
  static const uint64_t expected[SPOOKY_HASH_RESULTS_BUFSIZE] = {
  ...
  uint8_t buf[SPOOKY_HASH_RESULTS_BUFSIZE];
...
int spooky_hash_test_alignment() {
  printf("Testing alignment...");

  char buf[SPOOKY_HASH_ALIGNMENT_BUFSIZE];
  uint64_t hash[8];
  for(int i = 0; i < SPOOKY_HASH_ALIGNMENT_BUFSIZE - 16; ++i) {
    for(int j = 0; j < 8; ++j) {
      buf[j] = (char)(i + j);
      for(int k = 1; k <= i; ++k) {
        buf[j + k] = (char)k;
...
int main(MARK_UNUSED int argc, MARK_UNUSED char *argv[]) {
  if(spooky_hash_test_results()) {
    return EXIT_FAILURE;
  }
  if(spooky_hash_test_alignment()) {
    return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}